...
async function main() {
  ...
  const MINT_AMOUNT = BigInt(10_000_000_000_000); // 10,000 tokens with 9 decimals
  await mintTo(
    connection,
    keypair,
    tokenMint,
  ...
  const DEPOSIT_AMOUNT = new BN(1_000_000_000_000); // 1,000 tokens
  ...
  const amounts = [
    new BN(300_000_000_000), // 300 tokens to player 1 (30%)
    new BN(200_000_000_000), // 200 tokens to player 2 (20%)
    new BN(150_000_000_000), // 150 tokens to player 3 (15%)
  ];
  ...
  const WITHDRAW_AMOUNT = new BN(100_000_000_000); // 100 tokens
